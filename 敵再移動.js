...
var RepeatMoveAutoAction = defineObject(MoveAutoAction,
{
	...
	_isTargetMovable: function() {
		...
		if (StateControl.isBadStateOption(this._unit, BadStateOption.BERSERK)) {
			return false;
		}

		if (StateControl.isBadStateOption(this._unit, BadStateOption.AUTO)) {
			return false;
		}
	...
	_createSecondCombination: function() {
		...
		if (patternType === PatternType.APPROACH) {
			if (this._firstCombination.targetUnit) {
				...
				this._secondCombination = CombinationManager.getEscapeMoveCombination(this._unit, targetUnitPos, this._firstCombination);
			}
			else {
				this._secondCombination = CombinationManager.getEscapeMoveCombination(this._unit, firstCombination.targetPos, this._firstCombination);
			}
		}
